...
impl<T, U> ConvInto<U> for T
    where
        U: ConvFrom<T>
{
    fn conv_into(self) -> U { <U as ConvFrom<T>>::conv_from(self) }
}
...
impl<T, A> ConvFrom<(A,)> for [T; 1]
    where
        A: Into<T>,
{
    fn conv_from(t: (A,)) -> Self { [t.0.into()] }
}
...
impl<T, A> ConvFrom<[T; 1]> for (A)
    where
        T: Into<A> + Copy,
{
    fn conv_from(t: [T; 1]) -> Self { (t[0].into()) }
}
...
impl<T, A, B> ConvFrom<(A, B)> for [T; 2]
    where
        ...
        B: Into<T>,
{
    fn conv_from(t: (A, B)) -> Self { [t.0.into(), t.1.into()] }
}
...
{
    fn conv_from(t: [T; 2]) -> Self { (t[0].into(), t[1].into()) }
}
...
impl<T, A, B, C> ConvFrom<(A, B, C)> for [T; 3]
    where
        ...
        C: Into<T>,
{
    fn conv_from(t: (A, B, C)) -> Self { [t.0.into(), t.1.into(), t.2.into()] }
}
...
{
    fn conv_from(t: [T; 3]) -> Self { (t[0].into(), t[1].into(), t[2].into()) }
}
